...
export class FormDependentComponent {
  ...
  private dialogData = inject<{user:Dependent, formType:'waitingList' | 'dependent'}>(DIALOG_DATA)
 ...
 setDendentData(dependentData:{user:any, formType:'waitingList' | 'dependent'}){
  if(dependentData){
    this.userData = dependentData.user
    this.formType = dependentData.formType
    dependentData.user.birthDate = this.formatarData(dependentData.user.birthDate)
  ...
  updateDependent(){
    ...
    this.serviceData.updateDependent(this.dialogData.user.userId, this.dialogData.user.id ,formData).subscribe({
      next: (response) => {
  ...
  approveDependent(){
    ...
    this.serviceData.approveDependent(dependentData.id)
    .subscribe({
      next:()=>{
      ...
      error:(error)=>{
        this.updateMessage = {status:'error', message:`Ocorreu um erro, tente novamente cod: ${error.status}`}
        throw error
      }
    })
  }
  deleteDependent(){
    const dependentData = this.dialogData.user
    if(confirm(`Deseja EXCLUIR o dependente: ${dependentData.name} do funcionário(a): ${dependentData.user.name} ?` )){
    
    this.serviceData.deleteDependent(dependentData.userId, dependentData.id)
    .subscribe({
      next:()=>{
